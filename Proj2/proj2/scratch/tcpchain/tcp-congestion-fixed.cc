 ...
 *
 */
#include "tcp-congestion-fixed.h"
...
/**
 * \brief Tcp Fixed slow start algorithm
 *
...
/**
 * \brief Fixed congestion avoidance
 *
...
/**
 * \brief Try to increase the cWnd following the Fixed specification
 *
 * \see SlowStart
                         ...
                         uint32_t bytesInFlight)
{
  ...
  return state->m_initialCWnd * state->m_segmentSize;
...
void 
TcpFixed::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt){
  tcb->m_cWnd = tcb->m_initialCWnd * tcb->m_segmentSize;
}

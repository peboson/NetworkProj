...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("SocketType", "Tell echo applications to log if true", SocketType);
  cmd.AddValue ("InitialCwnd", "Tell echo applications to log if true", InitialCwnd);
  cmd.Parse (argc,argv);
  //if cmd line gives true then log level info
  if(SocketType=="TcpNewReno"){
    Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpNewReno::GetTypeId ()));
  }
  if(SocketType=="TcpFixed"){
